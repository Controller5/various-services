import streamlit as st

st.set_page_config(
        page_title="Various Services",
)
st.title('Various Services')

center_h = st.number_input('Center of the work should be:', 'step=int')
height = st.number_input('Height of Work', 'step=int')
width = st.number_input('Width of Work', 'step=int')   
from_top = st.number_input('Frame Allowance', 'step=int')
# from_edge = st.number_input('From Edge')
space_width = st.number_input('Space Width', 'step=int')

center_wall = space_width/2
screw_height = (center_h + (height/2)) - from_top 

st.write('The Screw Height should be ' + str(screw_height) + ' inches')
st.write('The Center of the Wall is ' + str(center_wall) + ' inches')
